...
저장소를 에이전트가 탐색하기 좋은 형태로 스캐폴딩하는 Agent Skill 의 원본 저장소
...
| `skills/repo-scaffold/` | 이 저장소가 배포하는 스킬의 원본 |
| `skills/repo-scaffold/assets/` | **대상 저장소로 복사되는 템플릿.** 이 저장소의 규칙이 아니다 |

인덱스에 같은 파일명이 두 번 나오는 것은 정상이다. `docs/standards/documentation.md` 는 이
저장소가 지키는 규칙이고 `skills/repo-scaffold/assets/docs/documentation.md` 는 남에게 깔아줄
템플릿이다. 규칙을 찾을 때는 앞의 것을 읽고, 배포물을 고칠 때는 뒤의 것을 고친다.
**둘은 자동으로 동기화되지 않는다.** 한쪽만 고치면 이 저장소가 옛 판으로 자기를 검사한다.
