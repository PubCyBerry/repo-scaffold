...
| `.codex/hooks.json` | Codex 가 Claude Code 와 다른 규칙으로 돈다. 같은 훅 스크립트를 물려 규칙을 한 벌로 유지한다 |
| `scripts/agent-hooks/` | 도구 선택, 게이트 우회, 채팅 답변처럼 diff 에 안 남는 규칙에 검사기가 없다. 규약이 있는데 아무 일도 안 일어난다 |

### 에이전트 훅이 규칙을 정의하지 않는 이유

`post-tool-use.sh` 는 확장자를 보고 `tests/*.sh` 를 부르기만 한다. 커밋 훅이 부르는 것과
같은 파일이다. 훅에 규칙을 다시 쓰면 두 곳이 갈리고 갈린 것을 아무도 보고하지 않는다.
훅이 바꾸는 것은 규칙이 아니라 트리거다. 커밋 때 알던 위반을 편집 직후에 안다.

`pre-tool-use.sh` 와 `stop.sh` 만 규칙을 갖는다. 그쪽은 파일에 안 남는 것들이라
커밋 훅이 애초에 볼 수 없다. 무엇을 강제하고 무엇을 안 하는지는 대상 저장소의
