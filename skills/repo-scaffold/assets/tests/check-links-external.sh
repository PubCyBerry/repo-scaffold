...
#
# 도구 없이 URL 을 보려면 tests/check-docs.sh 의 urls 단계가 curl 로 같은 일을 한다.
# 그쪽은 응답 코드만 보고 캐시도 재시도도 하지 않는다. 권위 있는 답은 이 스크립트가 낸다.
#
