...
Both run over the workflow directory through
[tests/check-workflows.sh](../../tests/check-workflows.sh), which the `workflow-lint` hook
invokes before each commit and CI runs on every push.
...
Configure Dependabot with a 7-day cooldown and grouped updates. The cooldown keeps a freshly
published release from landing the day it appears; grouping keeps one pull request per
ecosystem instead of one per package.
